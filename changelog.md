...
Fanbox 的 API 变化导致了此问题，现在修复。

这次是文章分页网址 `/post.paginateCreator` 里添加了一层 pageUrls 属性。
...
上次是文章数据里添加了一层 post 属性，这次是文章列表里添加了一层 posts 属性。
...
具体原因是文章数据的 body 里额外多了一层 post 属性，post 属性内部的就是原本的数据。
...
默认的颜色主题是白色，如果你想使用暗色主题的话需要修改此设置。
...
下载器会获取用户当前的赞助等级，然后获取创作者为当前等级设置的粉丝卡图片。但有些创作者并没有为每个等级设置粉丝卡图片，缺少了这项数据，导致下载器加载图片时出错。现在修复。
...
我的 Pixiv 下载器是有俄语文本的，而 Fanbox 下载器之前没有俄语文本。这因为 pixiv.net 官方有俄语界面，而 fanbox.cc 没有。我的下载器是与官方的语言选项保持一致的。

但是现在我发现 pixiv.net 不知何时取消了俄语界面，无语了。我干脆给 Fanbox 下载器也加上了俄语文本，现在是与 Pixiv 下载器保持一致的。
